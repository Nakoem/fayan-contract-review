...
items1 = [
    ('Agent架构：', '设计10工具ReAct自主决策循环，自动完成"提取条款→查法规→逐条分析→完整性检查→生成报告"，每种合同配备法定红线标准。'),
    ('RAG全链路：', '66条知识→Chroma向量库（216chunks），DashScope Embedding语义检索。效果："押金不退"命中"押金退还"（关键词0%→语义91%）。'),
    ('模型选型：', '对比测试4个模型（qwen-plus/max/3.6-plus/deepseek-v3.2），定义4维评测标准，选qwen-plus。'),
    ('工程化：', 'FastAPI REST + Docker + loguru日志 + Prompt YAML版管 + 三维度自动化评估（格式/召回率/LLM裁判）。'),
...
run = p.add_run('Python  ·  FastAPI  ·  Streamlit  ·  Docker  ·  Chroma  ·  MCP SDK  ·  DashScope  ·  Claude Code')
run.font.size = Pt(9)
run.font.name = '微软雅黑'
run.font.color.rgb = GOLD
...
jobs = [
    ('贵州创壹佳信息服务有限公司', '风控专员', '2025.01 - 2026.03', [
...
skills = [
    ('AI/Agent', 'ReAct Agent · Function Calling · RAG语义检索 · Prompt工程 · MCP协议 · LLM-as-Judge'),
